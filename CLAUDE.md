...
- 4 roles dentro do tenant: `viewer` (1) < `agent` (2) < `manager` (4) = `admin` (4) no acesso geral. A escada não decide tudo: `team.manage` pertence a manager/admin, `lead.delete` a manager/plataforma, e `ai.credentials.delete`, `organization.data.reset` e `api.tokens.manage` a admin/plataforma. São capacidades nomeadas porque ações irreversíveis não podem ser inferidas do rank geral
- Super-admin de plataforma é uma role transversal — `is_platform_admin` (decisão final na Spec 01)
- MFA TOTP é **opcional e ligado por quem administra** — não é mais forçado por papel. Quem exige são duas políticas independentes que SOMAM: `platform_admins.mfa_required` (para o super-admin) e `organizations.settings.security.mfa_required` (para `manager` e `admin` do tenant). O padrão de ambas é **não exigir**, e o `bootstrap-owner.ts` grava `false` explícito. Regra pura em `lib/auth/politica-mfa.ts`
