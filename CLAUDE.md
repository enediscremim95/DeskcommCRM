...
- 4 roles dentro do tenant: `viewer` (1) < `agent` (2) < `manager` (4) = `admin` (4) no acesso geral. A escada não decide tudo: `team.manage` e `lead.delete` são capacidades nomeadas exclusivas de `manager` e `is_platform_admin`; `admin` da organização não as recebe
