...
function renderPainel() {
  ...
  return {
    ...view,
    mudarConversa: (next: React.ComponentProps<typeof CRMSidePanel>["conversation"]) =>
      view.rerender(
        <QueryClientProvider client={client}>
          <CRMSidePanel conversation={next} />
        </QueryClientProvider>,
      ),
  };
}
...
describe("desfecho — rascunho atravessa somente lacuna transitória do mesmo contexto", () => {
  const resposta = (revision: number) => ({
    ...RESPOSTA,
    demandas: [{ ...RESPOSTA.demandas[0], revision }],
  });
  it("null→mesma conversa conserva escolha e revisão observada; conflito não renova CAS", async () => {
    ...
    expect(((await screen.findByLabelText("Desfecho da demanda")) as HTMLSelectElement).value).toBe(
      "perdida",
    );
    ...
    await waitFor(() =>
      expect(patch).toHaveBeenCalledWith("/api/v1/demandas/d-1", {
        action: "encerrar",
        desfecho: "perdida",
        expected_revision: 1,
      }),
    );
    expect((screen.getByLabelText("Desfecho da demanda") as HTMLSelectElement).value).toBe(
      "perdida",
    );
    ...
    await userEvent.click(screen.getByRole("button", { name: "Confirmar desfecho" }));
    await waitFor(() =>
      expect(patch).toHaveBeenLastCalledWith("/api/v1/demandas/d-1", {
        action: "encerrar",
        desfecho: "resolvida",
        expected_revision: 2,
      }),
  ...
  it.each(["conversa", "contato"])(
    "troca real de %s descarta rascunho, sem exibir nem reutilizar",
    async (kind) => {
      get.mockResolvedValue({ data: resposta(1) });
      const view = renderPainel();
      await userEvent.click(await screen.findByRole("button", { name: "Encerrar demanda" }));
      ...
      view.mudarConversa(
        kind === "conversa"
          ? { ...conversation!, id: "cv-2" }
          : {
              ...conversation!,
              contact_id: "outro",
              contacts: { ...conversation!.contacts!, id: "outro" },
            },
      );
