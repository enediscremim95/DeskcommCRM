...
3. ✅ **Follow-up como invariante de lead — DESILHADO (2026-07-24).** Era ilha parcial: o anti-morte era forte no *engine* (`schedule-followup.ts`, `followup-turn.ts`, `cron/scheduler.ts`, `edge/crm/session-watchdog.ts`), mas invisível para o humano. Fechado com o **Radar de Risco** (`/app/radar`): a montagem compartilhada continua classificando crítico / em risco / em voo para a API e a capacidade MCP; a tela humana mostra somente alertas que pedem ação agora, tarefas vencidas ou nas próximas 24 horas e leads frios sem tarefa, retorno ou compromisso. Arestas: **entrada** = `crm_leads` (abertos, `last_activity_at`) + `cron_jobs` e `crm_tasks`; **saída** = abrir o lead/contato, concluir a tarefa ou marcar follow-up sem sair do Radar. Código: `lib/leads/risk-radar.ts` (classificação pura + teste), `app/api/v1/leads/at-risk/route.ts`, `app/app/radar/`. Provado em tela por `tests/e2e/risk-radar.spec.ts`. (invariantes 4 + 5)
